...
workflow {
    ...
    align_reads_to_sars2_genome(trimming_reads.out)
}
...
process align_reads_to_sars2_genome {
    ...
    input:
    tuple val(sampleId), val(trimmed_reads)

    output:
    path "${sampleId}.bam", emit: sars2_aligned_reads_ch

    """
    bwa index ${params.SARS2_FA}
    bwa mem -t ${task.cpus} ${params.SARS2_FA} ${trimmed_reads[0]} ${trimmed_reads[2]} | samtools view -bF 4 - | samtools sort - > ${sampleId}_paired.bam
    bwa mem -t ${task.cpus} ${params.SARS2_FA} <(cat ${trimmed_reads[1]} ${trimmed_reads[3]}) | samtools view -bF 4 - | samtools sort - > ${sampleId}_unpaired.bam
