...
batch_size=${5:-'5'}
# batch_size=${5:-'500'}
profile=${6:-'codonslurm'}
snapshot_date=${7:-'2023-06-29'}  #2022-09-26 2022-10-24 2022-11-21 2022-12-19
...
echo "bq $table_name total rows: $row_count"
row_count=75000
############################################
...
for (( batch_index=skip; batch_index<skip+num_of_jobs&&batch_index<batches; batch_index++ )); do
  # mkdir -p "${root_dir}/${pipeline}_${batch_index}"; cd "${root_dir}/${pipeline}_${batch_index}" || exit
    ...
    sbatch -N 2 -p standard --mem 4096 --export ALL -t 4-00:00:00 "${DIR}/run.nextflow.sh" "${input_dir}/${table_name}_${batch_index}.tsv" \
