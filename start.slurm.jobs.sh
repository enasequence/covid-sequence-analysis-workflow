...
root_dir=${4:-"/project/${PROJECT_ID}/nextflow"} 
# batch_size=${5:-'15000'}
batch_size=${5:-'5'}
profile=${6:-'slurm'}
...
for (( batch_index=skip; batch_index<skip+num_of_jobs&&batch_index<batches; batch_index++ )); do
  ...
  # srun -N 2 --account=$PROJECT_ID -p small --export ALL --output "${DIR}/log/output.txt" "${DIR}/test.nextflow.sh" "${input_dir}/${table_name}_${batch_index}.tsv" \
  #   "${pipeline}" "${profile}" "${root_dir}" "${batch_index}" "${snapshot_date}" "${test_submission}"

  sbatch -N 2 --account=$PROJECT_ID -p small --export ALL --output "${DIR}/log/slurm-%j.out" --wrap="${DIR}/test.nextflow.sh ${input_dir}/${table_name}_${batch_index}.tsv \
    ${pipeline} ${profile} ${root_dir} ${batch_index} ${snapshot_date} ${test_submission}"
