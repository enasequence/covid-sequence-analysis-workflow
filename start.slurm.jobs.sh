...
profile=${6:-'lumislurm'}
snapshot_date=${7:-'2023-06-02'}  #2022-09-26 2022-10-24 2022-11-21 2022-12-19
...
for (( batch_index=skip;batch_index<skip+num_of_jobs&&batch_index<batches; batch_index++ )); do
  ...
  if [ "$profile" = "lumislurm" ]; then
    sbatch --export=ALL --account=$PROJECT_ID -N 12 --cpus-per-task=128 --mem='32GB' -t 48:00:00 -p standard --wrap="${DIR}/run.nextflow.sh ${input_dir}/${table_name}_${batch_index}.tsv \
      ${pipeline} ${profile} ${root_dir} ${batch_index} ${snapshot_date} ${test_submission}"
  fi

  if [ "$profile" = "codonslurm" ]; then
    sbatch --export ALL -N 2 -p standard --mem 4096 -t 4-00:00:00 "${DIR}/run.nextflow.sh" "${input_dir}/${table_name}_${batch_index}.tsv" \
      "${pipeline}" "${profile}" "${root_dir}" "${batch_index}" "${snapshot_date}" "${test_submission}"
  fi
  break
done
