...
for (( batch_index=skip;batch_index<skip+num_of_jobs&&batch_index<batches; batch_index++ )); do
  ...
  # hq submit --cpus 16 ${DIR}/hq.nextflow.sh ${input_dir}/${table_name}_${batch_index}.tsv \
  #   ${pipeline} ${profile} ${root_dir} ${batch_index} ${snapshot_date} ${test_submission}
  sbatch --account=$PROJECT_ID -N 4 --cpus-per-task=64 -t 72:00:00 -p small --export ALL --wrap="${DIR}/hq.nextflow.sh ${input_dir}/${table_name}_${batch_index}.tsv \
    ${pipeline} ${profile} ${root_dir} ${batch_index} ${snapshot_date} ${test_submission}"
done
