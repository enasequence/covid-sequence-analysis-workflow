...
########################################################
echo ""
echo "** Updating ${dataset_name}.submission_receipts_test table. **"
...
gsutil -m cp "${output_dir}/${snapshot_date}_${pipeline}_${batch_index}_receipts.tsv" \
  ...
  --autodetect "${dataset_name}.submission_receipts_test" "gs://${dataset_name}/${snapshot_date}_${pipeline}_${batch_index}_receipts.tsv" \
  "analysis_accession:STRING,file_submitted:STRING,time_submitted,snapshot_date"

sql="UPDATE ${dataset_name}.submission_receipts_test SET snapshot_date = '""${snapshot_date}""' WHERE snapshot_date is NULL"
bq --project_id="${project_id}" --format=csv query --use_legacy_sql=false "${sql}"
sql="CREATE OR REPLACE TABLE ${dataset_name}.submission_receipts_test AS SELECT DISTINCT * FROM ${dataset_name}.submission_receipts_test"
bq --project_id="${project_id}" --format=csv query --use_legacy_sql=false "${sql}"
